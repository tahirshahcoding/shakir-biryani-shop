...
export const GET = withErrorHandling(async () => {
  ...
  const [todaySales, todayExpenses, recentSales, topProductsData, lowStockItems, totalProducts] =
    await Promise.all([
      db.sale.aggregate({
      ...
      db.$queryRawUnsafe<{ name: string; currentQuantity: number; minimumQuantity: number; unit: string }[]>(
        `SELECT name, "currentQuantity", "minimumQuantity", unit
         FROM "InventoryItem"
         WHERE "isActive" = true
           AND "currentQuantity" <= GREATEST("minimumQuantity", COALESCE(
             NULLIF((SELECT "value"::numeric FROM "Setting" WHERE "key" = 'LOW_STOCK_THRESHOLD'), 0), 0
           ))
