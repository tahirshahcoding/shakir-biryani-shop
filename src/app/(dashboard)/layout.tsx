...
const navItems = [
  ...
  { label: "POS", href: "/pos", icon: "cart" },
  { label: "Sales", href: "/sales", icon: "receipt" },
  { label: "Products", href: "/products", icon: "box" },
  { label: "Categories", href: "/categories", icon: "tag" },
  { label: "Inventory", href: "/inventory", icon: "archive" },
  { label: "Expenses", href: "/expenses", icon: "dollar" },
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <aside className="hidden lg:fixed lg:inset-y-0 lg:flex lg:w-64 lg:flex-col bg-white border-r border-gray-200">
        <div className="flex h-16 items-center px-6 border-b border-gray-200">
          <h1 className="text-lg font-bold text-orange-600">Shakir Biryani</h1>
        </div>
      ...
      {/* Mobile header — just the title, no hamburger */}
      <div className="lg:hidden sticky top-0 z-40 bg-white border-b border-gray-200 safe-top">
        <div className="flex h-12 items-center justify-between px-4">
          <h1 className="text-base font-bold text-orange-600">Shakir Biryani</h1>
          <button
            onClick={handleLogout}
      ...
      {/* Mobile bottom nav — all tabs, scrollable */}
      <div className="lg:hidden fixed bottom-0 inset-x-0 z-40 bg-white border-t border-gray-200 safe-bottom">
        <nav className="flex overflow-x-auto scrollbar-hide" aria-label="Navigation">
          {navItems.map((item) => {
            const isActive = pathname === item.href || pathname.startsWith(item.href + "/");
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`flex flex-col items-center gap-0.5 px-3 py-2 text-[10px] font-medium whitespace-nowrap transition-colors shrink-0 ${
